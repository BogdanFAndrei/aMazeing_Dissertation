...
class BFS(_CellDraw):
    def __init__(self, start, goal, visited, animate, screen):
        ...
        self.openset = queue.Queue()  # Initialize openset as a queue
    ...
    def get_openset(self):
        ...
        for neighbor in neighbors:
            ...
            print("Appending to openset", self.openset.qsize())
    ...
    def evaluation(self):
        ...
        if not self.path_time:
            if not self.openset.empty():
                print("Found openset in evaluation")
                self.current_cell = self.openset.get()  # Dequeue the first cell from the frontier
                print("Current cell:", self.current_cell)
                print("Exploring neighbors:")
                self.close_setting.add(self.current_cell)  # Mark the current cell as visited
                ...
                for cell in self.get_neighbors():
                    print("Adding neighbor:", cell)
                    if cell not in self.close_setting:
                        self.openset.put(cell)
